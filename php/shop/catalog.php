...
$queryTopProducts = $conn->query("SELECT `products`.`id`, `products`.`nameProduct`, `products`.`priceProduct`, `products`.`imageProduct` , SUM(`reviewsStars`) AS `totalStars`, COUNT(*) AS `reviewCount`, (SUM(`reviewsStars`) / COUNT(*)) AS `rating` FROM `reviews` JOIN `products` ON `reviews`.`reviewsProductID` = `products`.`id` GROUP BY `products`.`id` ORDER BY `rating` DESC LIMIT 3")
?>